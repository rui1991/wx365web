        ...
        <el-main class="module-main">
          <div class="search">
            <div class="search-input" style="margin-bottom: 10px;">
              ...
              <div class="operate">
                ...
                <el-button type="primary" v-if="authority.add" :disabled="addOperate" @click="addClick">新增</el-button>
              </div>
            </div>
            <div class="search-input">
              ...
              <div class="operate">
                ...
                <el-button type="primary" :disabled="downDisabled" v-if="authority.add" @click="downFile">导出</el-button>
              </div>
            </div>
          </div>
          <el-table class="list-table" :data="tableData" border style="width: 100%">
            ...
            <el-table-column label="操作" width="178">
              <template slot-scope="scope" v-if="opeUser">
                ...
                <a href="javascript:void(0);" class="operate com" @click="comClick(scope.row.user_id, scope.row.card_mac)">编辑</a>
...
export default{
  ...
  methods: {
    ...
    getClientId () {
      ...
      this.$axios({
        method: 'post',
        url: this.sysetApi() + '/v3.2/selParentCom',
        data: params
      }).then((res) => {
        if (res.data.result === 'Sucess') {
        } else {
    ...
    getRoleOptions () {
      ...
      this.$axios({
        ...
        if (res.data.result === 'Sucess') {
          ...
          let ids = []
          let roles = []
          roleData.forEach((item, index, array) => {
            if (ids.indexOf(item.role_id) === -1) {
    ...
    getOrganData () {
      ...
      this.$axios({
        ...
        if (res.data.result === 'Sucess') {
          ...
          if (this.sectorDialog) {
            setTimeout(() => {
              this.$refs.sectorTree.setCheckedKeys([])
            }, 100)
          } else if (this.accreditDialog) {
            const ids = this.comForm.accreditId.split(',')
            setTimeout(() => {
    ...
    addAccClick () {
      ...
      if (this.accreditTree.length === 0) {
        this.getOrganData()
      } else {
        const ids = this.addForm.accreditId.split(',')
        setTimeout(() => {
          this.$refs.accreditTree.setCheckedKeys(ids)
        }, 100)
      }
    ...
    selectAccredit () {
      ...
      const clentNodes = nodesData.filter(item => {
        return item.organize_type === 1
      })
      if (firmNode) {
        ids.push(firmNode.id)
        names.push(firmNode.name)
      } else if (clentNodes.length > 0) {
        clentNodes.forEach(node => {
          ids.push(node.id)
