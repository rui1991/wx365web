<template>
  <div>
    <!-- 编辑 -->
    <el-dialog title="编辑网关" :visible.sync="parentDialog" :show-close="false" :close-on-click-modal="false" custom-class="medium-dialog">
      <el-form class="divide-from" :model="formData" :rules="rules" ref="ruleForm" :label-width="formLabelWidth">
        ...
        <el-form-item label="网关ID" prop="id">
          <el-input :disabled="true" v-model.trim="formData.id" auto-complete="off"></el-input>
        </el-form-item>
...
export default{
  ...
  methods: {
    ...
    sendRequest () {
      ...
      this.$axios({
        method: 'post',
        url: this.loraApi() + '/lora/altGw/' + this.parentId,
        data: params
      }).then((res) => {
        this.disabled = false
