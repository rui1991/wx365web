<template>
  <el-dialog title="提示" :visible.sync="parentDialog" :show-close="false" :close-on-click-modal="false" custom-class="hint-dialog">
    <p class="hint-text"><i class="el-icon-warning"></i>是否要删除该车辆？</p>
...
/*
* type: 0  添加车辆   1编辑车辆   2删除车辆
* */
export default{
  props: ['parentDialog', 'parentId', 'parentCarNumber'],
  data () {
    return {
      disabled: false
    }
  ...
  methods: {
    ...
    submitForm () {
      let params = {
        ...
        id: this.parentId,
        car_number: this.parentCarNumber
      }
