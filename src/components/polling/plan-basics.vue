...
export default{
  ...
  methods: {
    /*
    * 逻辑说明：
    *   执行类型：1.执行人员    2.执行组   （默认为执行人员）
    *   执行部门：在执行类型为执行人员的时候需选择
    *   执行组：在执行类型为执行组的时候需选择
    *   选择日期：任务的开始和结束日期（在编辑计划的时候开始日期必须大于当天）
    *   任务模式：1.单次任务   0.循环任务 （默认为单次任务）
    *           单次任务：在执行类型为执行人员的情况下可选择一个执行人
    *           循环任务：可选择执行频次
    *   执行频次：一天多次的时候可选择周期，其余情况不可选择周期；（默认一天一次）
    *   周期：可多选，当需要选择周期时至少选择一个
    *   时间段：一天多次的时候需要选择多个时间段，当执行类型为执行人员的时候可以指派执行人员
    * */
